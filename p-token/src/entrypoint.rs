...
pub unsafe extern "C" fn entrypoint(input: *mut u8) -> u64 {
    ...
    /// have zero data, which is being validated before the offset
    /// is used.
    const EXPECTED_INSTRUCTION_DATA_LEN_OFFSET: usize = 0x7a78;
        ...
        && (*input.add(ACCOUNT3_HEADER_OFFSET) == 255)
    {
        ...
        let offset = EXPECTED_INSTRUCTION_DATA_LEN_OFFSET + account_3_data_len_aligned;
            ...
            if likely(discriminator == 3) {
                let instruction_data = unsafe {
                    from_raw_parts(
                        ...
                        input.add(offset + size_of::<u64>() + size_of::<u8>()),
                        size_of::<u64>(),
                    )
                };
